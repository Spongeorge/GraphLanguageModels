"""This code is adapted from the Huggingface Transformers library."""
...
class T5Attention(nn.Module):
    ...
    @staticmethod
    def _og_relative_position_bucket(
    ...
    def _relative_position_bucket(
        ...
        relative_buckets = T5Attention._og_relative_position_bucket(
    ...
    def compute_bias(
        ...
        if relative_position is None:
            ...
            assert (
                relative_position.dtype == torch.long
            ), f"{relative_position.dtype} should be torch.long"
            assert (
                relative_position.device == device
            ), f"{relative_position.device} should be {device}"
        ...
        values = values.permute([2, 0, 1]).unsqueeze(
            0
        )  # shape (query_length, key_length)  # [MP] actually the shape is (1, num_heads, query_length, key_length)
    ...
    def forward(
        ...
        if position_bias is None:
                    ...
                    position_bias = torch.cat(
                        tuple(
                            self.compute_bias(
                                ...
                                device=scores.device,
                                relative_position=r,
                                use_additional_bucket=u,
                            )
                            for r, u in zip(relative_position, use_additional_bucket)
                        ),
                        dim=0,
                    )
            ...
            logging.debug(
                f"position_bias = {position_bias.shape if position_bias  is not None else position_bias}"
            )
        ...
        if sparsity_mask is not None:
            assert (
                sparsity_mask.dtype == torch.bool
            ), f"{relative_position.dtype} should be torch.bool"
        ...
        # >>> masked softmax >>>
        if sparsity_mask is not None:
            scores[sparsity_mask] = float(
                "-inf"
            )  # this works in the backward pass, because potential nan values that the softmax produces in the forward pass are not used in backpropagation, because the "=" is independent of the value that the entry had previously. This is not the case for "+=", which is why we need to set the values to -inf instead of adding -inf.
    ...
    def init_relative_position_bias(
        self,
        modelsize: str,
        is_decoder: bool = False,
        init_additional_buckets_from: list[int] = None,
    ):
        """
        ...
        logging.debug(
            "Loading model from which relative position bias should be inherited"
        )
        parent_model = T5EncoderModel.from_pretrained(
            modelsize
        )  # Needs to be changed for decoder support

        logging.debug("Get relative position bias from parent model")
        if is_decoder:
            parent_bias = (
                parent_model.decoder.block[0]
                .layer[0]
                .SelfAttention.relative_attention_bias.weight
            )
        else:
            parent_bias = (
                parent_model.encoder.block[0]
                .layer[0]
                .SelfAttention.relative_attention_bias.weight
            )
        ...
        assert (
            parent_bias.shape[1] == self.relative_attention_bias.weight.shape[1]
        ), f"{parent_bias.shape[1]} should be {self.relative_attention_bias.weight.shape[1]}"
        assert (
            parent_bias.shape[0] <= self.relative_attention_bias.weight.shape[0]
        ), f"{parent_bias.shape[0]} should be <= {self.relative_attention_bias.weight.shape[0]}"
        ...
        num_additional_buckets = (
            self.relative_attention_bias.weight.shape[0] - parent_bias.shape[0]
        )
        ...
        assert (
            len(init_additional_buckets_from) == num_additional_buckets
        ...
        init_additional_buckets_from = [
            0 if idx is None else idx for idx in init_additional_buckets_from
        ]
        init_additional_buckets_from = torch.tensor(
            init_additional_buckets_from, dtype=torch.long
        )
        ...
        with torch.no_grad():
            for i, (skip, idx) in enumerate(
                zip(skip_bucket, init_additional_buckets_from),
                start=parent_bias.shape[0],
            ):
                if skip:
                    continue
...
class T5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def forward(
        ...
        if concept_indices is not None:  # aggregate concept embeddings of encoder
            ...
            # assert False
            for b, tmp_batch_concept_indices in enumerate(
                concept_indices
            ):  # iterate over batch
                for (
                    tmp_concept_indices
                ) in tmp_batch_concept_indices.values():  # iterate over concepts
                    tmp_indices = torch.tensor(
                        [
                            [i for i in range(*one_concept_occurance)]
                            for one_concept_occurance in tmp_concept_indices
                        ],
                        device=hidden_states.device,
                    ).T  # shape (token-length-of-concept, num-occurances-of-concept)

                    for (
                        tmp_tok_indices
                    ) in tmp_indices:  # iterate over tokens within each concept
                        hidden_states[b, tmp_tok_indices] = torch.index_select(
                            hidden_states[b], 0, tmp_tok_indices
                        ).mean(
                            dim=0, keepdim=True
                        )  # save mean over all occurances

                    attention_mask[b, tmp_indices[:, 1:]] = (
                        False  # only attend to first occurace
                    )
...
class T5EncoderModel(T5PreTrainedModel):
    ...
    def init_relative_position_bias(
        ...
        if init_decoder:
            self.decoder.block[0].layer[0].SelfAttention.init_relative_position_bias(
                modelsize=modelsize,
                is_decoder=init_decoder,
                init_additional_buckets_from=init_additional_buckets_from,
            )
        else:
            self.encoder.block[0].layer[0].SelfAttention.init_relative_position_bias(
                modelsize=modelsize,
                is_decoder=init_decoder,
                init_additional_buckets_from=init_additional_buckets_from,
            )
