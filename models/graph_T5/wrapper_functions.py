...
class Graph:
    ...
    def __init__(self, g: List[Tuple[int, str, int]] = [], node_labels: dict = {}):
        ...
        )  # list of all relations in the graph, including duplicate relations
...
def _get_graphT5_relativeposition_sparsitymask(
    ...
    relative_position = torch.zeros(
        size=(sequence_length, sequence_length), dtype=torch.long
    )
...
def _get_shortest_path_graphT5_relativeposition_sparsitymask(
    g: nx.Graph, sequence_length: int, use_eos: bool, eos: str
) -> Tuple[torch.Tensor, torch.Tensor]:
    ### get relative position of each node in the sequence, as well as the sparsity mask ###
    # initialize relative position matrix)
...
def graph_to_graphT5(g: nx.Graph, tokenizer: T5Tokenizer, how: str, eos: str) -> Data:
    ...
    assert eos in [
        ...
        "bidirectional",
        "unidirectional",
    ], f"{eos = } must be either 'False', 'bidirectional', or 'unidirectional'"
    ...
    if how == "local":
        relative_position, sparsity_mask, use_additional_bucket = (
            _get_graphT5_relativeposition_sparsitymask(
        ...
        num_additional_buckets = (
            1  # gGLM uses 1 additional bucket for long-ranged G2G connections
        )
        ...
        raise ValueError(f"how must be either 'local', 'global', or 'shortest_path', but is {how}")
...
def _get_set_of_triplets_input_sequence(
    g: Graph, str2tok: dict, order: str
) -> Tuple[list, dict]:
    # get input sequence (i.e. sequence that will be fed into the model for this graph)
    all_nodes = (
    ...
    }  # dictionary mapping each node to its start-index and end- in the sequence. Keys are nodes, values are lists of tuples (start_index, end_index). The lists have a length of #degree(concept) for concepts and are as long as the number of occurances of the relation in the graph for relations.

    index_counter = 0
    sequence = []
    is_concept = (
        []
    )  # list of length #tokens, where each element is True if the token is a concept and False if it is a relation
    ...
    if order == "random":
        ...
        raise ValueError(
            f"order must be either 'random' or 'alphabetical', but is {order}"
        )
...
def graph_to_set_of_triplets(
    ...
    sequence, indices, is_concept, concept_indices = (
        _get_set_of_triplets_input_sequence(g, str2tok, order=order)
    )  # get input sequence (i.e. sequence that will be fed into the model for this graph

    input_ids = sequence
    relative_position = (
        None  # None means that the encoder is a normal sequence transformer
    )
...
def _get_relative_position(size):
    return torch.tensor(
        [[i - j for i in range(size)] for j in range(size)], dtype=torch.long
    )
...
def get_embedding(
    ...
    """
    assert (
        concept in indices.keys()
    ), f"{concept = } is not a node in the graph. {indices = }"
    assert (
        len(indices[concept]) == 1
    ), f"{concept = } is not a concept, as concepts occur only once in the graph. {indices = }"
...
def add_text_to_graph_data(data, text, tokenizer, use_text):
    ...
    new_relative_position = torch.zeros(
        size=(1, new_seq_len, new_seq_len), dtype=data.relative_position.dtype
    )
    new_relative_position[:, :old_seq_len, :old_seq_len] = data.relative_position
    new_relative_position[:, old_seq_len:, old_seq_len:] = _get_relative_position(
        text_seq_len
    )

    new_sparsity_mask = torch.zeros(
        size=(1, new_seq_len, new_seq_len), dtype=data.sparsity_mask.dtype
    )
    new_sparsity_mask[:, :old_seq_len, :old_seq_len] = data.sparsity_mask
    new_sparsity_mask[:, old_seq_len:, old_seq_len:] = True

    new_use_additional_bucket = torch.zeros(
        size=(1, new_seq_len, new_seq_len), dtype=data.use_additional_bucket.dtype
    )
    new_use_additional_bucket[:, :old_seq_len, :old_seq_len] = (
        data.use_additional_bucket
    )
    new_use_additional_bucket[:, old_seq_len:, old_seq_len:] = (
        False  # could change that if we want T2T and local G2G relations to be learned separately
    )
    ...
    if use_text == "FullyConnected":
        ...
        new_relative_position[:, old_seq_len:, :old_seq_len] = (
            data.num_additional_buckets
        )
        new_relative_position[:, :old_seq_len, old_seq_len:] = (
            data.num_additional_buckets + 1
        )
